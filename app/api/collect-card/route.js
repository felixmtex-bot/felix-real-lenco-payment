...
export default function CardPage(){
  ...
  const [card,setCard]=useState({number:"",exp:"",cvv:"",name:"",email:""});
  ...
  const pay=async()=>{
    ...
    setLoading(true); setMsg("Processing Visa card...");
    try{
      ...
      if(j.success){setMsg("✅ Card payment successful!"); setTimeout(()=>{ if(returnUrl) window.location.href=returnUrl+"?paid=1"; else window.location.href="/success"; },1000); return;}
    ...
    <div style={{background:"#0a0a0a",minHeight:"100vh",color:"#fff",fontFamily:"Inter",padding:16}}>
      <div style={{maxWidth:440,margin:"0 auto"}}>
        ...
        <div style={{background:"#151515",borderRadius:16,padding:16,border:"1px solid #222"}}>
          ...
          <input value={card.number} onChange={e=>setCard({...card,number:e.target.value})} placeholder="Card Number 4242 4242 4242 4242" style={{width:"100%",background:"#0a0a0a",border:"1px solid #333",padding:14,borderRadius:12,marginTop:10,color:"#fff",outline:"none"}}/>
          ...
          {msg && <div style={{marginTop:10,padding:10,background:"#1a1a1a",borderRadius:10,fontSize:12,textAlign:"center"}}>{msg}</div>}
          ...
          <p style={{fontSize:10,color:"#555",textAlign:"center",marginTop:10}}>Secured by Lenco • Visa • Mastercard</p >
        </div>
      </div>
    </div>
  )
